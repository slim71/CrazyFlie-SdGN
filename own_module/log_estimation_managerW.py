...
class LogEstimationManager:
    ...
    def __init__(self, cf):
        self._logconf = LogConfig(name='Estimation', period_in_ms=100)
        self._cf = cf
    ...
    def callback(self, timestamp, data):
        print('[%d][%s]: %s' % (timestamp, self._logconf.name, data))
    ...
    def simple_log_async(self):
        self._cf.log.add_config(self._logconf)    # check if variables are allowed (i.e. are in TOC)
