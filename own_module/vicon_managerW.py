...
class ViconManager:
        ...
        while not got_frame:  # Continue until the stream mode is set and working
            try:
                ...
                logging.debug("Fetched! Pulled frame number: %d" if got_frame
                              else "Vicon is not streaming!", self._client.GetFrameNumber())
            except ViconDataStream.DataStreamException as exc:
    ...
    def get_drone_pos(self):    # TODO check vicon vs crazy frame
        drone_trans = self._client.GetSegmentGlobalTranslation(Drone, 'Crazyflie')
    ...
    def enable_data(self):  # Enable all data types
        ...
        logging.debug('Segments', self._client.IsSegmentDataEnabled())
        logging.debug('Markers', self._client.IsMarkerDataEnabled())
        logging.debug('Unlabeled Markers', self._client.IsUnlabeledMarkerDataEnabled())
        logging.debug('Marker Rays', self._client.IsMarkerRayDataEnabled())
        logging.debug('Devices', self._client.IsDeviceDataEnabled())
        logging.debug('Centroids', self._client.IsCentroidDataEnabled())

    def set_reference_system(self):
        try:
            self._client.SetAxisMapping(ViconDataStream.Client.AxisMapping.EForward,
