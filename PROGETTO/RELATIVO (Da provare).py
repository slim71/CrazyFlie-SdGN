...
MAKE_LOG_FILE = 0                           #Set to 0 if you don't want to obtain the log file after the execution. Otherwise set to 1. The log file can be used by the matlab file "plot.m"
...
LOG_TEST_WITH_DISACTIVATED_THRUSTER = 0     #Set to 1 if you want to do a test without make the Drone fly. In that case we suggest to set the other flags to these values:
...
def create_Matrix_Rotation(client, Drone, D_T_meters, last_gamma,KALMAN_INCLUDE_QUATERNION):
    ...
    #In case we update the last value of the yaw angle (not null value) we update returning it toghether with the matrixes:
...
if MAKE_LOG_FILE:
    ...
    filename = "Log_File_CrazyFlie_RELATIVE" + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    #Thruster activated:
        ...
        with MotionCommander(scf, MOTION_COMMANDER_DEFAULT_HEIGHT) as mc:
            ...
            while(height_drone < DEFAULT_HEIGHT):
                ...
                Matrix_homogeneous, Matrix_Rotation, last_gamma = create_Matrix_Rotation(client, Drone, First_Position, last_gamma, KALMAN_INCLUDE_QUATERNION)
            ...
            #We maintain separate the variables dedicated to the Kalman update (last_drone_position) and the setpoint update (last_drone_setpoint)
            last_drone_setpoint = np.array([D_T_meters[0], D_T_meters[1], D_T_meters[2]])
                ...
                if(take_off==0):
                    try:
                        ...
                        if(W_T_meters[0]==0 and W_T_meters[1]==0 and W_T_meters[2]==0):
                            ...
                            cf.commander.send_position_setpoint(last_drone_setpoint[0], last_drone_setpoint[1], last_drone_setpoint[2], 0)
                            ...
                            if(CONSECUTIVE_LOSS==MAX_LOSS):
                                ...
                                landing(last_drone_setpoint,SUBTRACTED_HEIGHT,DELTA_HEIGHT,cf,lg_stab,fdesc,MAKE_LOG_FILE)
                        else:

                            if(FIRST_ITERATION or CONSECUTIVE_LOSS):
                            #At the first iteration the translation has to be a null vector because there isn't a early value to use for the difference.
                            ...
                            #this isn't the first iteration and didn't have been losses
                                ...
                                #We update the value to use in the next transaltion:
                                W_T_prec = np.array([W_T_meters[0], W_T_meters[1], W_T_meters[2]])

                            CONSECUTIVE_LOSS = 0

                            if(FIRST_ITERATION==0):
                                #We convert the translation of the wand in body frame in order to use it for computing the next setpoint for the drone
                                Wand_Traslation = np.dot(Matrix_Rotation, np.transpose(Wand_Traslation))

                            #We compute the new setpoint as sum of the last setpoint and the new translation of the wand.
                            last_drone_setpoint[0] = last_drone_setpoint[0] + Wand_Traslation[0]
                            last_drone_setpoint[1] = last_drone_setpoint[1] + Wand_Traslation[1]
                            last_drone_setpoint[2] = last_drone_setpoint[2] + Wand_Traslation[2]

                            #we are no more in the first iteration of this loop since  this line is executed for the first time
                            FIRST_ITERATION=0
                            ...
                            if KALMAN_INCLUDE_QUATERNION:
                            #In this case we don't have to compute again the matrix for the rotation because it is constant.
